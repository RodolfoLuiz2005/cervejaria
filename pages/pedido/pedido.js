...
document.addEventListener('DOMContentLoaded', () => {
  ...
  renderTrackPage();
  trackTimer = setInterval(renderTrackPage, 2000);
  App.safeIcons();
});
...
function renderTrackPage() {
  ...
  document.getElementById('app').innerHTML = `<section class="container page-pad" style="max-width:52rem"><div class="card-surface rounded-2xl p-6"><div class="top-row"><div><div class="text-xs uppercase tracking muted">Acompanhar pedido</div><h1 class="font-display text-3xl text-gradient-gold">${App.escapeHTML(pedido?.codigo || 'Sem pedido')}</h1></div><div class="flex gap-2"><button class="chip ${lookup.mode === 'latest' ? 'active' : ''}" data-mode="latest">Mais recente</button><button class="chip ${lookup.mode === 'codigo' ? 'active' : ''}" data-mode="codigo">Por código</button><button class="chip ${lookup.mode === 'mesa' ? 'active' : ''}" data-mode="mesa">Por mesa</button></div></div><form id="lookup-form" class="mt-4 flex gap-2 ${lookup.mode === 'latest' ? 'hidden' : ''}"><input id="lookup-input" class="input" placeholder="${lookup.mode === 'codigo' ? 'Ex.: PED-001' : 'Ex.: Mesa 01'}" value="${App.escapeHTML(lookup.value || '')}"><button class="btn btn-outline" type="submit">Buscar</button></form>${pedido ? `<div class="mt-4 text-sm muted">Cliente: <span class="price">${App.escapeHTML(pedido.cliente || '-')}</span> • ${pedido.tipo === 'delivery' ? `Delivery` : App.escapeHTML(pedido.mesa || 'Mesa')}</div>` : `<p class="muted mt-4">Nenhum pedido encontrado para os critérios atuais.</p>`}<ol class="status-steps">${steps.map((s, i) => `<li class="status-step ${pedido && i <= idx ? 'active' : ''}"><div class="status-icon">${pedido && i === idx ? '<span class="animate-shimmer" style="position:absolute;inset:0;border-radius:999px"></span>' : ''}<i data-lucide="${s[2]}"></i></div><span class="status-label">${s[1]}</span></li>`).join('')}</ol><div class="mt-8"><h2 class="font-display text-sm uppercase tracking muted">Itens</h2><ul class="cart-list mt-2">${itens.map(i => `<li class="flex justify-between text-sm" style="padding:.5rem 0;border-bottom:1px solid rgba(74,83,96,.65)"><span>${Number(i.quantidade || 0)}x ${App.escapeHTML(i.nome || '')}</span><span class="price">${App.formatBRL(Number(i.preco || 0) * Number(i.quantidade || 0))}</span></li>`).join('') || '<li class="muted text-sm" style="padding:.5rem 0">Sem itens para exibir.</li>'}</ul></div>${pedido ? `<div class="mt-4 flex items-center justify-between"><span class="muted">Status atual: <span class="price">${App.escapeHTML(statusLabel(pedido.status))}</span></span><span class="price">${App.formatBRL(pedido.total || 0)}</span></div>` : ''}<a href="../cliente/index.html" class="mt-6" style="display:inline-block;color:var(--gold)">Voltar ao cardápio</a></div></section>`;
