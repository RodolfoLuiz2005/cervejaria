...
document.addEventListener('DOMContentLoaded', () => {
  ...
  renderKitchen();
  kitchenTimer = setInterval(() => renderKitchen(false), 2000);
  App.safeIcons();
});
...
async function renderKitchen(animate = true) {
  if (rendering) return;
  rendering = true;
  try {
    ...
    for (const col of cols) {
      ...
      board.insertAdjacentHTML('beforeend', `<div class="column"><div class="column-head"><div class="column-title"><i data-lucide="${col[2]}" style="color:var(--gold)"></i>${col[1]}</div><span class="count">${list.length}</span></div><div class="space-y">${list.map(p => card(p, animate)).join('') || '<div class="empty">Vazio</div>'}</div></div>`);
    }
    if (highlightedOrderIds.size) setTimeout(() => highlightedOrderIds.clear(), 1800);
