...
function renderAdmin(animate = true) {
  ...
  if (adminTab === 'dashboard') renderDashboard(animate);
  if (adminTab === 'produtos') renderProdutos();
  if (adminTab === 'promocoes') renderPromocoes();
  if (adminTab === 'pedidos') renderPedidos(animate);
  if (adminTab === 'faturamento') renderFaturamento(animate);
...
function renderDashboard(animate = true) {
  ...
  document.getElementById('admin-content').innerHTML = `<div class="flex items-center justify-between gap-2"><h2 class="font-display text-lg">Dashboard</h2><button id="reset-dashboard" class="btn btn-outline">Resetar números</button></div><p class="text-xs muted mt-2">Último reset: ${App.escapeHTML(resetAt)}</p><div class="admin-kpi-grid mt-4 ${animate ? 'motion-stagger' : ''}"><article class="card-surface rounded-xl p-4"><div class="label">Pedidos do dia</div><div class="font-display text-3xl" data-count-to="${m.day.pedidos}">${animate ? '0' : m.day.pedidos}</div></article><article class="card-surface rounded-xl p-4"><div class="label">Faturamento do dia</div><div class="font-display text-3xl" data-count-to="${m.day.faturamento}" data-count-type="currency">${animate ? App.formatBRL(0) : App.formatBRL(m.day.faturamento)}</div></article><article class="card-surface rounded-xl p-4"><div class="label">Pedidos da semana</div><div class="font-display text-3xl" data-count-to="${m.week.pedidos}">${animate ? '0' : m.week.pedidos}</div></article><article class="card-surface rounded-xl p-4"><div class="label">Faturamento da semana</div><div class="font-display text-3xl" data-count-to="${m.week.faturamento}" data-count-type="currency">${animate ? App.formatBRL(0) : App.formatBRL(m.week.faturamento)}</div></article><article class="card-surface rounded-xl p-4"><div class="label">Pedidos do mês</div><div class="font-display text-3xl" data-count-to="${m.month.pedidos}">${animate ? '0' : m.month.pedidos}</div></article><article class="card-surface rounded-xl p-4"><div class="label">Faturamento do mês</div><div class="font-display text-3xl" data-count-to="${m.month.faturamento}" data-count-type="currency">${animate ? App.formatBRL(0) : App.formatBRL(m.month.faturamento)}</div></article></div><div class="mt-6"><h3 class="font-display text-sm uppercase tracking muted">Histórico de reset</h3><ul class="admin-reset-history">${history.map(item => `<li>${App.escapeHTML(new Date(item.at).toLocaleString('pt-BR'))}</li>`).join('') || '<li class="muted">Sem histórico.</li>'}</ul></div>`;
  const resetBtn = document.getElementById('reset-dashboard');
  if (resetBtn) resetBtn.onclick = resetDashboard;
  if (animate) App.animateNumbers(document.getElementById('admin-content'));
}
...
function renderPedidos(animate = true) {
...
function renderFaturamento(animate = true) {
  const r = reportData();
  document.getElementById('admin-content').innerHTML = `<div class="flex items-center justify-between gap-2"><h2 class="font-display text-lg">Relatórios</h2><button id="download-report" class="btn btn-outline">Baixar relatório</button></div><div class="admin-kpi-grid mt-4 ${animate ? 'motion-stagger' : ''}"><article class="card-surface rounded-xl p-4"><div class="label">Pedidos do dia</div><div class="font-display text-3xl" data-count-to="${r.day.pedidos}">${animate ? '0' : r.day.pedidos}</div></article><article class="card-surface rounded-xl p-4"><div class="label">Faturamento do dia</div><div class="font-display text-3xl" data-count-to="${r.day.faturamento}" data-count-type="currency">${animate ? App.formatBRL(0) : App.formatBRL(r.day.faturamento)}</div></article><article class="card-surface rounded-xl p-4"><div class="label">Pedidos da semana</div><div class="font-display text-3xl" data-count-to="${r.week.pedidos}">${animate ? '0' : r.week.pedidos}</div></article><article class="card-surface rounded-xl p-4"><div class="label">Faturamento da semana</div><div class="font-display text-3xl" data-count-to="${r.week.faturamento}" data-count-type="currency">${animate ? App.formatBRL(0) : App.formatBRL(r.week.faturamento)}</div></article><article class="card-surface rounded-xl p-4"><div class="label">Pedidos do mês</div><div class="font-display text-3xl" data-count-to="${r.month.pedidos}">${animate ? '0' : r.month.pedidos}</div></article><article class="card-surface rounded-xl p-4"><div class="label">Faturamento do mês</div><div class="font-display text-3xl" data-count-to="${r.month.faturamento}" data-count-type="currency">${animate ? App.formatBRL(0) : App.formatBRL(r.month.faturamento)}</div></article></div><div class="card-surface rounded-xl p-4 mt-6 ${animate ? 'motion-slide-up' : ''}"><h3 class="font-display text-lg">Produtos mais vendidos</h3><ul class="mt-3" style="padding-left:1rem">${r.top.map(([name, q]) => `<li>${App.escapeHTML(name)} - <span class="price">${q}</span></li>`).join('') || '<li class="muted">Sem vendas registradas.</li>'}</ul></div><div class="card-surface rounded-xl p-4 mt-4 ${animate ? 'motion-slide-up' : ''}"><h3 class="font-display text-lg">Pedidos por status</h3><ul class="mt-3" style="padding-left:1rem">${Object.entries(r.byStatus).map(([status, qty]) => `<li>${App.escapeHTML(statusLabel(status))}: <span class="price">${qty}</span></li>`).join('') || '<li class="muted">Sem pedidos.</li>'}</ul></div>`;
